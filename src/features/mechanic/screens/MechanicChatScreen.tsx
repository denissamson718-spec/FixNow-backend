...
export function MechanicChatScreen() {
  const navigation = useNavigation<any>();
  const { activeRequest, markJobMessagesRead, registeredAccounts, sendJobMessage } = useAppContext();
  const driverAccount = registeredAccounts.find(
  ...
  return (
    <Screen>
      <View style={styles.header}>
        <Pressable accessibilityLabel="Close chat" onPress={() => navigation.navigate("Jobs")} style={styles.iconButton}>
          <Ionicons name="close" size={24} color={palette.ink} />
        </Pressable>
        <View style={styles.headerCopy}>
          <Text numberOfLines={1} style={styles.title}>{driverAccount?.profile.fullName || "Driver"}</Text>
          <Text numberOfLines={1} style={styles.subtitle}>{activeRequest.vehicle || activeRequest.issue || "Active job"}</Text>
        </View>
        <Pressable accessibilityLabel="Video call driver" onPress={handleVideoCall} style={styles.videoButton}>
          <Ionicons name="videocam" size={20} color={palette.primaryDark} />
        </Pressable>
        <Pressable accessibilityLabel="Call driver" onPress={handleCall} style={styles.callButton}>
      ...
      <View style={styles.thread}>
        {(activeRequest.messages ?? []).length ? (
          (activeRequest.messages ?? []).map((message) => (
            <View key={message.id} style={[styles.bubble, message.senderRole === "mechanic" && styles.bubbleMine]}>
              <Text style={styles.sender}>{message.senderRole === "mechanic" ? "You" : message.senderName}</Text>
      ...
      <View style={styles.composer}>
        ...
        <TextInput value={messageDraft} onChangeText={setMessageDraft} style={styles.input} placeholder="Message driver" placeholderTextColor={palette.inkSoft} multiline />
